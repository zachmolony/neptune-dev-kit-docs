...
The `cartSize` function returns the number of products in the cart. It accepts no parameters.
...
const CartPage = () => {
  ...
  return (
    <div>
      ...
      <p>Cart is empty: {isEmpty() ? "Yes" : "No"}</p>
      ...
      <ul>
        {cart.items.map((item) => (
          <li key={item.id}>
            ...
            <button onClick={() => incrementProductQuantity(item.id)}>+</button>
            <button onClick={() => decrementProductQuantity(item.id)}>-</button>
          </li>
        ))}
      </ul>
    </div>
